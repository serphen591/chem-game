(() => {
  ...
  const fullText = (value) => {
    if (value == null) return '未记录';
    const text = typeof value === 'string' ? value : JSON.stringify(value);
    return String(text).replace(/\s+/g, ' ').trim() || '未记录';
  };
  const wrap = (value, width = 25, lines = 3, showAll = false) => {
    const text = showAll ? fullText(value) : compact(value, width * lines);
    const result = [];
    for (let index = 0; index < text.length && (showAll || result.length < lines); index += width) result.push(text.slice(index, index + width));
    return result;
  };
  const textLines = (value, x, y, options = {}) => {
    const rows = options.rows || wrap(value, options.width || 25, options.lines || 3, options.full === true);
  ...
  function createSnapshot(context = {}) {
    ...
    const messageRows = wrap(context.message || '本步骤操作不正确', 20, 1, true);
    const actualRows = wrap(actual, 43, 1, true);
    const expectedRows = wrap(expected, 45, 1, true);
    const messageBottom = 246 + Math.max(0, messageRows.length - 1) * 26;
    const stepBoxY = Math.max(370, messageBottom + 34);
    const leftBottom = stepBoxY + 104;
    const actualBottom = 227 + Math.max(0, actualRows.length - 1) * 24;
    const operationCardHeight = Math.max(58, actualBottom - 181 + 22);
    const iconLineY = Math.max(291, 181 + operationCardHeight + 52);
    const iconCenterY = iconLineY + 3;
    const benchBottom = iconLineY + 54;
    const targetY = benchBottom + 19;
    const targetTextY = targetY + 54;
    const targetTextBottom = targetTextY + Math.max(0, expectedRows.length - 1) * 23;
    const targetBottom = Math.max(targetY + 91, targetTextBottom + 28);
    const svgHeight = Math.max(500, leftBottom, targetBottom + 25);
    const panelHeight = svgHeight - 131;
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="900" height="${svgHeight}" viewBox="0 0 900 ${svgHeight}">
      ...
      <rect width="900" height="${svgHeight}" rx="28" fill="url(#bg)"/>
      ...
      <rect x="24" y="105" width="258" height="${panelHeight}" rx="22" fill="#fff" stroke="#d7e8e0"/>
      ...
      ${textLines(context.message || '本步骤操作不正确', 45, 246, {rows:messageRows,size:16,color:'#c93643',weight:800,gap:26})}
      <rect x="45" y="${stepBoxY}" width="216" height="78" rx="15" fill="#f4f8f6"/>
      <text x="61" y="${stepBoxY + 25}" fill="#718079" font-size="11" font-weight="800" font-family="Microsoft YaHei,sans-serif">步骤编号</text>
      ${textLines(context.stepKey || 'unknown', 61, stepBoxY + 51, {width:21,lines:1,size:15,weight:900})}
      <rect x="300" y="105" width="576" height="${panelHeight}" rx="22" fill="#fff" stroke="#d7e8e0"/>
      <text x="325" y="140" fill="#13251f" font-size="20" font-weight="900" font-family="Microsoft YaHei,sans-serif">当时的实验界面</text>
      <rect x="325" y="158" width="526" height="${benchBottom - 158}" rx="18" fill="url(#bench)"/>
      <rect x="345" y="181" width="486" height="${operationCardHeight}" rx="14" fill="#ffffffdd" stroke="#cfe2d9"/>
      <text x="365" y="203" fill="#718079" font-size="11" font-weight="800" font-family="Microsoft YaHei,sans-serif">学生实际操作</text>
      ${textLines(actual, 365, 227, {rows:actualRows,size:16,weight:900,gap:24})}
      <line x1="345" y1="${iconLineY}" x2="831" y2="${iconLineY}" stroke="#62b790" stroke-width="7" stroke-linecap="round"/>
      <circle cx="390" cy="${iconCenterY}" r="25" fill="#fff" stroke="#86cdb1" stroke-width="3"/><path d="M380 ${iconCenterY - 14}h20v29c0 10-20 10-20 0z" fill="#a6e4cb" stroke="#3f7665" stroke-width="2"/>
      <circle cx="470" cy="${iconCenterY}" r="25" fill="#fff" stroke="#86cdb1" stroke-width="3"/><path d="M458 ${iconCenterY - 12}h24l-4 28h-16z" fill="#dcefeb" stroke="#3f7665" stroke-width="2"/>
      <circle cx="550" cy="${iconCenterY}" r="25" fill="#fff" stroke="#86cdb1" stroke-width="3"/><path d="M544 ${iconCenterY - 17}h12v31h-12z" fill="#f4c563" stroke="#8a6327" stroke-width="2"/>
      <rect x="325" y="${targetY}" width="526" height="${targetBottom - targetY}" rx="16" fill="#eef8f3"/>
      <text x="345" y="${targetY + 26}" fill="#08785a" font-size="12" font-weight="900" font-family="Microsoft YaHei,sans-serif">正确目标（完成实验后解锁回看）</text>
      ${textLines(expected, 345, targetTextY, {rows:expectedRows,size:15,weight:800,gap:23})}
    </svg>`;
    return { version:4, kind:'error-interface', mime:'image/svg+xml', width:900, height:svgHeight, dataUrl:toDataUrl(svg) };
  }
  ...
  function eventEvidence(event = {}, fallback = {}) {
    ...
    const stored = payload.snapshot && Number(payload.snapshot.version) >= 4 && /^data:image\/svg\+xml;base64,/.test(payload.snapshot.dataUrl || '') ? payload.snapshot : null;
